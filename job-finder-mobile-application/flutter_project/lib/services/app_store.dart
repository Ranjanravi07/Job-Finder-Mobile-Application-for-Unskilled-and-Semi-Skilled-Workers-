...
class AppStore extends ChangeNotifier {
  ...
  /// The OTP most recently generated by [sendOtp]. Random per request so the
  /// login flow behaves like a real backend instead of a fixed code.
  String _currentOtp = '';
  ...
  // ---------------------------------------------------------------
  void sendOtp(Function(String) onError, Function(String) onSuccess) {
    if (phone.length < 10) {
      onError(lang == 'ne' ? 'कृपया १० अंकको सहि मोबाइल नम्बर हाल्नुहोस्।' : 'Please enter a valid 10-digit mobile number.');
      return;
    }
    // Simulated backend: generate a fresh random 6-digit OTP per request.
    _currentOtp = (Random().nextInt(900000) + 100000).toString();
    onSuccess(_currentOtp);
  }

  void verifyOtp(String otpCode, Function(String) onError, Function(bool, bool) onVerified) {
    if (otpCode.trim().isEmpty) {
      onError(lang == 'ne' ? 'कृपया ओटिपी कोड हाल्नुहोस्।' : 'Please enter the OTP code.');
      return;
    }
    if (otpCode.trim() != _currentOtp) {
      onError(lang == 'ne' ? 'गलत ओटिपी कोड।' : 'Invalid OTP.');
      return;
    }
    ...
    workers = workers.map((w) {
      if (w.id == profileId) {
        return w.copyWith(
          ...
          govId: updated['govId'] as String? ?? w.govId,
  ...
  void createEmployerProfile({
    ...
    final newProfile = EmployerProfile(
      ...
      govId: (govId ?? '').isNotEmpty ? govId! : 'CIT-12345-NP',
      govIdFiles: govIdFiles ?? [],
    ...
    employers = employers.map((e) {
      if (e.id == profileId) {
        return e.copyWith(
          ...
          govId: updated['govId'] as String? ?? e.govId,
